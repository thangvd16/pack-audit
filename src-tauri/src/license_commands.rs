...
fn no_license() -> LicenseStatus {
    LicenseStatus {
        valid: false,
        needs_refresh: false,
        message: "Không tìm thấy mã bản quyền".into(),
        days_remaining: 0,
    }
}
...
pub async fn activate_license_online(
    ...
    if !status.is_success() {
        let msg = body["error"]
            .as_str()
            .unwrap_or("Không thể kích hoạt mã bản quyền")
            .to_string();
        return Err(msg);
    }
    ...
    Ok(LicenseStatus {
        valid: true,
        needs_refresh: false,
        message: format!("Còn {} ngày sử dụng", days_remaining),
        days_remaining,
    })
}

#[tauri::command]
pub async fn refresh_license_online(app: tauri::AppHandle) -> Result<LicenseStatus, String> {
    let path = license_file_path(&app);
    let content =
        std::fs::read_to_string(&path).map_err(|_| "Không tìm thấy file bản quyền".to_string())?;
    let saved = parse_license_file(&content).ok_or("File bản quyền bị lỗi")?;

    let did = device_id();
    if saved.device_id != did {
        return Err("Thiết bị không khớp".into());
    }
    ...
    if !status.is_success() {
        let msg = body["error"]
            .as_str()
            .unwrap_or("Không thể xác minh lại mã bản quyền")
            .to_string();
        return Err(msg);
    }
...
#[tauri::command]
pub fn check_saved_license(app: tauri::AppHandle) -> LicenseStatus {
    let path = license_file_path(&app);
    ...
    if saved.device_id != did {
        return LicenseStatus {
            valid: false,
            needs_refresh: false,
            message: "Thiết bị không khớp".into(),
            days_remaining: 0,
        };
    }
